<template>
    <b-card :header="__('messages.plan')" no-body>
        <b-card-body>
            <b-form-row class="mb-3">
                <b-col align="center">
                    <b-form-checkbox v-model="value.allowBlocks" button
                                     :button-variant="value.allowBlocks ? 'success' : 'secondary'">
                        {{ __("messages.allow") }}
                        {{ __("messages.custom blocks") }}
                    </b-form-checkbox>
                    <b-form-checkbox @change="toggleUseOpeningBlock()" button
                                     :button-variant="useOpeningBlock ? 'success' : 'secondary'">
                        {{ __("messages.first paragraph") }}
                    </b-form-checkbox>
                    <b-form-checkbox @change="toggleUseClosingBlock" button
                                     :button-variant="useClosingBlock ? 'success' : 'secondary'">
                        {{ __("messages.last paragraph") }}
                    </b-form-checkbox>
                </b-col>
            </b-form-row>
            <b-form-row>
                <b-col>
                    <b-form-group>
                        <b-form-input name="from" type="number" min="0" v-model="value.sizes.from"
                                      :placeholder="__('messages.sizes.from')"
                                      v-validate="'required|integer|min_value:0|max:11'"
                                      :state="noErrors('from')"/>
                        <b-form-invalid-feedback>
                            {{ errors.first("from") }}
                        </b-form-invalid-feedback>
                    </b-form-group>
                </b-col>
                <b-col>
                    <b-form-group>
                        <b-form-input name="to" type="number" min="0" v-model="value.sizes.to"
                                      :placeholder="__('messages.sizes.to')"
                                      v-validate="'required|integer|min_value:0|max:11'"
                                      :state="noErrors('to')"/>
                        <b-form-invalid-feedback>
                            {{ errors.first("to") }}
                        </b-form-invalid-feedback>
                    </b-form-group>
                </b-col>
            </b-form-row>
            <block-component v-model="value.openingBlock" :plan-config="planConfig"
                             :position="planConfig.headings.opening"
                             @add-settings-block="addSettingsBlock(null, openingBlock)"
                             @add-keys-block="addKeysBlock(null, openingBlock)" v-if="useOpeningBlock"/>
            <block-component v-for="(block, index) in value.blocks" v-model="value.blocks[index]"
                             :key="'block-' + block.uid" :plan-config="planConfig" :position="index"
                             :previous-heading-type="previousHeadingType(index)" :next-same-index="nextSameIndex(index)"
                             :allow-blocks="value.allowBlocks" :previous-same-index="previousSameIndex(index)"
                             :blocks-length="blocksLength" @move-up="moveUp" @move-down="moveDown"
                             @add-before="addBefore" @add-after="addAfter" @add-settings-block="addSettingsBlock"
                             @add-keys-block="addKeysBlock" @add-child="addChild" @delete="deleteBlock" ref="blocks"/>
            <block-component v-model="value.closingBlock" :plan-config="planConfig"
                             :position="planConfig.headings.closing"
                             @add-settings-block="addSettingsBlock(null, closingBlock)"
                             @add-keys-block="addKeysBlock(null, closingBlock)"
                             v-if="useClosingBlock"/>
        </b-card-body>
    </b-card>
</template>
<script src="./plan.js"></script>